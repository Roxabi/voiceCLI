...
`load_defaults()` checks `~/.roxabi/voicecli/voicecli.toml` first, then walks up from CWD to `$HOME` as fallback. Place your config at `~/.roxabi/voicecli/voicecli.toml` for global access regardless of which project you run from. If not found, a warning is printed to stderr and built-in defaults are used.
