...
def mock_chime(monkeypatch):
    monkeypatch.setattr("voicecli.core.samples._chime", lambda *_: None)
...
class TestRecordSample:
    @patch("voicecli.core.samples.shutil.which", return_value="/usr/bin/parecord")
    def test_happy_path_timeout_is_normal_stop(self, _mock_which, samples_env, mock_chime):
        ...
        with patch("voicecli.core.samples.subprocess.run", side_effect=fake_run):
            # Act
    ...
    def test_nonzero_returncode_raises(self, _mock_which, samples_env, mock_chime):
        ...
        with patch("voicecli.core.samples.subprocess.run", return_value=mock_result):
...
class TestSamplesRecordCommand:
    def test_happy_path(self):
        ...
        with patch("voicecli.core.samples.record_sample", return_value=fake_dest):
    ...
    def test_runtime_error_exits_1(self):
        ...
        with patch(
            "voicecli.core.samples.record_sample",
...
class TestFromUrlValueErrorPrefix:
    def test_value_error_uses_invalid_argument_prefix(self):
        ...
        with patch(
            "voicecli.core.samples.from_url",
