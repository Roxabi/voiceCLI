...
flowchart LR
    ...
    subgraph Tests["tests/nats/"]
        ...
        T_VAL[test_envelope_validation.py NEW]
...
| Agent | Tasks | Files |
|---|---|---|
| tester-A | T1, T6, T10 (RED test authoring for new modules) | `tests/nats/test_envelope_validation.py`, `tests/nats/test_tts_runner.py`, `tests/nats/test_stt_runner.py` |
...
#### T1 [P=N] — RED: write `tests/nats/test_envelope_validation.py` covering every error branch
...
- **Difficulty:** 3
- **Time:** 25 min
- **Files:** `tests/nats/test_envelope_validation.py` (new)
- **Snippet:**
  ```python
  ...
  ```
- **Verify:** `uv run pytest tests/nats/test_envelope_validation.py --collect-only`
...
- **Snippet:**
  ...
  ```
- **Verify:** `uv run pytest tests/nats/test_envelope_validation.py`
...
- **Verify:**
  ...
  uv run pytest tests/nats/test_envelope_validation.py tests/nats/test_tts_runner.py tests/nats/test_stt_runner.py \
...
|------|---------------|-----------|---------|
| T1 | tester-A | — | RED: write `tests/nats/test_envelope_validation.py` covering all error branches |
...
- T1: 12 — RED: write tests/nats/test_envelope_validation.py covering all error branches
